    ...
    public partial class Main : Form
    {
        ...
        new ChartingActor(sysChart)), "charting");
            ...
            _toggleActors[CounterType.Cpu] = Program.ChartActors.ActorOf(
                Props.Create(() => new ButtonToggleActor(_coordinatorActor, MonitorCpuButton, CounterType.Cpu,
            ...
            _toggleActors[CounterType.Memory] = Program.ChartActors.ActorOf(
               Props.Create(() => new ButtonToggleActor(_coordinatorActor, MonitorMemoryButton,
                CounterType.Memory, false))
                .WithDispatcher("akka.actor.synchronized-dispatcher"));

            // DISK button toggle actor
            _toggleActors[CounterType.Disk] = Program.ChartActors.ActorOf(
               Props.Create(() => new ButtonToggleActor(_coordinatorActor, MonitorDiskButton, CounterType.Disk,
                false)).WithDispatcher("akka.actor.synchronized-dispatcher"));
        ...
        private void MonitorCpuButton_Click(object sender, EventArgs e)
        {
            _toggleActors[CounterType.Cpu].Tell(new ButtonToggleActor.Toggle());
        }

        private void MonitorMemoryButton_Click(object sender, EventArgs e)
        {
            _toggleActors[CounterType.Memory].Tell(new ButtonToggleActor.Toggle());
        }

        private void MonitorDiskButton_Click(object sender, EventArgs e)
        {
